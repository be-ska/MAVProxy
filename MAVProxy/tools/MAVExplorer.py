...
def cmd_graphs(args):
    ...
    matching_graphs = [g for g in mestate.graphs if graph_name.upper() in g.name.upper()]
    if not matching_graphs:
        print("No predefined graph found matching: %s" % graph_name)
        return
    if len(matching_graphs) > 1:
        print("Multiple graphs match '%s':" % graph_name)
        for g in matching_graphs:
            print("  %s" % g.name)
        return
    # Use the matching graph
    g = matching_graphs[0]
